...
class Product extends Model
{
    ...
    public function blog(){
        return $this->belongsToMany(Blog::class,'pr_products_blogs','product_id','blog_id')->withPivot([]);
    }
    ...
    public function scopeStatusFilter($query, $filter)
    {
        ...
        if (!isset($filter['customer_name']) || empty($filter['customer_name'])) {
            return $query;
        }
        $category_slugs = $filter['customer_name'];
        return $query->whereHas('customers', function ($query) use ($category_slugs) {
...
//        $list = $list->with(['customers:id,name'])->paginate($rows);
    ...
    {
        $item = self::with(['createdByUser', 'updatedByUser', 'customers', 'deletedByUser', 'blog' => function ($query) {
        ...
        $customer_slug = $item->customers->slug ?? null;

        $response = [
            'success' => true,
            'data' => array_merge($item->toArray(), [
                'customer_slug' => $customer_slug,
                'blog_id' => $item->blog->pluck('blog_id')->toArray(),
            ]),
        ];
    ...
    public static function validation($inputs)
    {
        ...
        $custom_messages = [
            'customer_id.required' => 'The customer ID field is required. Please provide a valid customer name.',
        ];
