...
let model_namespace = 'VaahCms\\Modules\\CustomerProject\\Models\\Product';


let base_url = document.getElementsByTagName('base')[0].getAttribute("href");
let ajax_url = base_url + "/customerproject/products";
...
export const useProductStore = defineStore({
    ...
    actions: {
        ...
        async refreshPage (){
            this.$router.push({name: 'products.index'}).then(() => {
                window.location.reload();
            });
        },
        //---------------------------------------------------------------------
        ...
        toEdit(item)
        {
            ...
            this.$router.push({name: 'products.form', params: { id: item.id, customer_id: this.customer_id } });
        },
        //---------------------------------------------------------------------
        isViewLarge()
        {
        ...
        async search(event) {
            ...
            await vaah().ajax(
                this.ajax_url + '/auto-complete/',
                this.searchAfter,
                options
            );
        },
        searchAfter(data, res) {
            if (data) {
                this.filtered_customers = data;
            }
        },
        handleCustomerSelect() {
            this.item.customer_id = this.item.customer_slug.id;
            // this.item.customer_slug = this.item.customer_slug.id;
        },
