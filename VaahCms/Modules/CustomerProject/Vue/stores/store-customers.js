...
let model_namespace = 'VaahCms\\Modules\\CustomerProject\\Models\\Customer';


let base_url = document.getElementsByTagName('base')[0].getAttribute("href");
let ajax_url = base_url + "/customerproject/customers";
...
export const useCustomerStore = defineStore({
    ...
    getters: {
        isUpdatePermission() {
            return this.assets.permission
                .includes('customerproject-can-read-only');
        },
        isDeletePermission(){
            return !this.assets.permission
                .includes('can-delete-jobs');
        }
    },
    actions: {
        ...
        redirectToProductTable(item) {
            this.$router.push({ name: 'products.index', query: { customer_name:item } });
        },
        ...
        async sendNotification(id) {
            ...
            await vaah().ajax(
                this.ajax_url + '/customer/notify',
                this.afterSendNotification,
                options
            );
        },
        afterSendNotification(data, res) {
            if (data) {
                console.log(data)
            }
        },
        ...
        {
            // const permissionIndex = this.assets.permission.indexOf('customerproject-can-update-customer');
            // const hasPermission = permissionIndex > -1;
            // console.error(hasPermission);

            return vaah().hasPermission(this.assets.permission, slug);
            // console.log(hasPermission(this.assets.permission, slug))
        },
